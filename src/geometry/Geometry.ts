...
class Geometry extends Drawable
{
    ...
    {
        let mesh = readTextFile('../objs/icosahedron.obj');
    ...
    extrude(face: Face, dist: number): void
    {
        ...
        {
            let edge1: vec3 = vec3.create();
            vec3.subtract(edge1, halfEdge.next.vertex.position, halfEdge.vertex.position);
            let edge2: vec3 = vec3.create();
            vec3.subtract(edge2, halfEdge.next.next.vertex.position, halfEdge.vertex.position);
            let crossProd: vec3 = vec3.create();
            vec3.cross(crossProd, edge1, edge2);
            if (crossProd == vec3.fromValues(0, 0, 0))
            {
            ...
            normal = vec3.normalize(crossProd, crossProd);
            break;
        }
